...
Traveler:
The demand-side user searching for, evaluating, requesting, booking, and managing a stay.

Provider:
The supply-side user responsible for operating, managing, or stewarding one or more listings.

Platform Admin:
The internal user responsible for curation, supply review, trust, and marketplace operations.
...
Notes:
- Date and traveler count are search intent inputs even if availability logic is simplified at first.
...
Outcome:
- A Booking Request exists and can be accepted, declined, or completed according to the transaction model.
...
2. Provider sees listings, upcoming reservations, pending requests, availability, and earnings summary.
