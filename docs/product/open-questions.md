...
| OQ-04 | Should collections become first-class ordered editorial entities? | Deferred. MVP uses `collectionTags` and filtered search links. | Before building custom CMS collection pages or ordered merchandising |
| OQ-05 | Which listing detail fields should become indexed filters next? | Deferred. MVP indexes only `stayType`, `maxTravelers`, and `collectionTags` beyond native search. | After observing Traveler search behavior |
