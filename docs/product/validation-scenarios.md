...
- At least one published Listing has location, price, images, availability, `stayType`,
  `maxTravelers`, `collectionTags`, bedrooms, beds, bathrooms, and amenities.
...
- `pub_stayType`, `pub_maxTravelers`, and `pub_collectionTags` appear only when their filters are
  active.
...
- Filtered search uses `publicData.collectionTags`.
...
No UI should describe `pending-payment`, `preauthorized`, `declined`, `expired`, or `payment-expired`
as a confirmed Reservation.
...
- Search query construction includes `pub_stayType`, `pub_maxTravelers`, and
  `pub_collectionTags` only for configured filters.
